...
class VoiceModelDefaultsTest {
    ...
    fun realtimeCallVoiceModelOrderShouldBeFixed() {
        assertEquals(
            listOf("QWEN_OMNI_PLUS", "DOUBAO", "GPT"),
            V88VoiceModelOptions.map { it.id }
        )
        assertEquals(
            listOf("QwenOmniPlus", "Seeduplex", "GPT Realtime2.0"),
        ...
        assertEquals(listOf(true, true, false), V88VoiceModelOptions.map { it.enabled })
    }

    @Test
    fun providerIdsShouldRoundTripToTheHomeSheetIds() {
        assertEquals("DOUBAO", "DOUBAO".toV88VoiceModelId())
        assertEquals("QWEN_OMNI_PLUS", "DOUBAO_SEEDUPLEX_3_0".toV88VoiceModelId())
        ...
        assertEquals("QWEN_OMNI_PLUS", "DOUBAO_SEEDUPLEX_3_0".toRealtimeCallProviderValue())
